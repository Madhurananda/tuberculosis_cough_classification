...
def mean_ROC(mean_tpr, mean_fpr, fname, eval_type):
    ...
    # print('auc_acc: ', auc_acc)
    

    # plt.plot(mean_fpr, mean_tpr, color='g', label='ROC curve (AUC = {})'.format(auc_acc))
    # plt.legend(loc='lower right')
    # plt.plot([0, 1], [0, 1], color='navy', linestyle='--')
    # plt.xlim([-0.05, 1.0])
    # plt.ylim([-0.05, 1.05])
    ...
    # try:
    #     fname = os.path.basename(fname)[:-4]
    #     fig_out_dir = '{}/{}'.format(ROC_mean_figs_dir, fname)
    #     if not os.path.isdir(fig_out_dir):
    #         os.mkdir(fig_out_dir)
    #     plt.savefig(fig_out_dir + '/ROC_{}.png'.format(eval_type))
    # except:
    #     plt.show()
    #     pass
    # plt.show()
...
for features_dir in features_dir_list:
    ...
    for k in range(1, N_Folds+1):
        print ('fold no: ', k)
        train_df, test_df = load_splits(full_df=data, f=features_dir, k=k)
    ...
    # mean_auc = mean_ROC(mean_tpr_ADS,mean_fpr, features_dir, 'ADS')
    # plt.plot(mean_fpr, mean_tpr_ADS, color='darkorange', label='ADS ROC curve (AUC = {:.4f})'.format(mean_auc))
    # mean_auc = mean_ROC(mean_tpr_TBI[1], mean_fpr, features_dir, 'TIS')
    # plt.plot(mean_fpr, mean_tpr_TBI[1], color='green', label='TIS ROC curve (AUC = {:.4f})'.format(mean_auc))
    # plt.legend(loc='lower right')
    # plt.plot([0, 1], [0, 1], color='navy', linestyle='--')
    # plt.xlim([-0.05, 1.0])
    # plt.ylim([-0.05, 1.05])
    ...
    tbi_scores_gamma = np.insert(tbi_scores_gamma, 3, mean_auc, axis=1)
