...
def cv_param_estimation_MLP(df):
    ...
    momentum_list = np.linspace(0, 1, num=21)
...
splits_dir_base = proj_database + 'feature_data/splits/'
...
for val_size in val_sizes: 
    ...
    for feature in features:
        ...
        saveFig_title = result_dir + 'ROC_figs/'+str(feature[0:len(feature)-4])+'_split='+str(val_size)+'.png'
        ...
        for k in range(1, N_FOLDS + 1):
            ...
            to_save_values_fold_name = result_dir + 'K-Fold_results/val-' +str(val_size)+ '_feat-' + str(feature[:-4])+ '_K-' +str(k)
            save_model_title = result_dir + 'models/'+str(feature[0:len(feature)-4])+'_split='+str(val_size)+'_K-' +str(k)
            ...
            # ## First, check if the calculation is already done for this fold or not
            # to_save_values_fold_name = '../data/LR_classifier/results/LR_K-Fold_results/val-' +str(val_size)+ '_feat-' + str(feature[:-4])+ '_K-' +str(k)
            ...
            # """
            # try:
            #     opt_LRM, to_save_params = cv_param_estimation(train_df)
            #     to_save += '\n' + to_save_params
            # except:
            #     now = datetime.now()
            #     current_time = now.strftime("%Y/%m/%d at %H:%M:%S")
            ...
            # This method didn't work. So, I have to find another way to save the method
            # for the German guys. 
            
            ## Assuming these are the best parameters 
            batch_size = 10
            epochs = 10
            weight_constraint = 1
            dropout_rate = 0.1
            ...
            y = list(df_.TB_status.values)
            X = df_[feat_names]
            model.fit(np.array(X), np.array(y))
            
            try: 
                os.mkdir(save_model_title) 
            except OSError as error: 
                print(error)  
            
            model.save(save_model_title+'/MLP_model')
            
            ## Then load the model this way 
            new_model = tf.keras.models.load_model(save_model_title+'/MLP_model')
